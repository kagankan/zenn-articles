...
| 利用時の品質 | 予測容易性     | 名前の意味を正しく当てられるか | 一貫・規約から次が読める                 |
| 利用時の品質 | 到達容易性     | 必要な人が名前にたどり着けるか | 既存 util の再利用・ライブラリ採用       |
| 利用時の品質 | 理解容易性       | 速く・楽に読み解けるか       | 短く一貫した名前ほど速い                 |
| 利用時の品質 | 伝達容易性     | 会話・AI 指示で指し示しやすいか | 一語で正確に指せる                     |
| 利用時の品質 | 安全性         | 誤用・バグを防げるか         | `deleteAt`/`deletedAt` を取り違えない    |
| 利用時の品質 | 安定性         | 将来も名前を変えずに済むか   | 実装でなく意図で名付ける                 |
| 利用時の品質 | 変更容易性     | 変えるとき楽・安全に直せるか | 一意な名前は grep で一括置換できる       |
| 利用時の品質 | 読み手網羅性   | 人・AI・新規参加者に通じるか | 誰が読んでも分かる                       |

以降で、各性質にどういった内容が含まれるか、そして具体的な改善事例を示します。また、特性どうしの **トレードオフ** についても後半で触れていきます。
...
以降のコード例で **🪫 は「その観点（〇〇性）が低い」、🔋 は「高い」** を表します。単にその観点の中での比較であり、絶対的な正解・不正解ではないことを示すために ✅／❌ を避けています。同じ名前でも観点ごとに高低は変わり、総合的な良し悪しは文脈（影響範囲や固定した前提）で決まります。
...
あえてこういった間違った命名をしようとする人はいないでしょうが、すでにあるコードを変更したときに、実装を変えたが命名を変え忘れたりすると名前と中身が乖離することはよくあります。
...
読み手が「この名前はこういう意味だろう」と *正しく* 当てられること。内部識別性・外部識別性・スタイル一貫性・スタイル適合性などが寄与する（定数が `UPPER_SNAKE` なら「定数だ」と当てられる、など。「誤解を招かない」はこの裏返しだ）。
<!-- 有効性 -->

### 到達容易性 (reachability)

必要な人が、その名前に *たどり着ける* こと。同僚が既存のユーティリティ関数を見つけて再利用する（重複実装を防ぐ）、使ってほしいライブラリが Web 検索で見つかって採用される、など。内部検索性・外部検索性が *見つけやすさ* を、明確性・予測容易性が「これが探していたものだ」という確信を与える。検索性が *見つけられる性質* なら、到達容易性は *実際にたどり着けた結果* だ（なお「発見可能性」はデザイン分野で「どんな操作が可能か・どう操作するかを見つけられるか」という別の意味で使われるため、ここでは *到達* と呼ぶ）。
<!-- 有効性 -->

### 理解容易性 (understandability)

*速く・楽に* 読み解けること。短く一貫した名前ほど速い。予測容易性が「正しく分かる」なら、こちらは「速く分かる」── 別の軸であり、簡潔さと明確さのように両者は衝突することもある。
<!-- 効率性 -->

### 伝達容易性 (communicability)

名前を *共有語彙* として、チームの会話・レビューや AI への指示で、正確かつ素早く指し示せること。一語で「`getExpiredDate` を直そう」と言えるか、AI に「`data` を意味のある名前に」と的確に指示できるか。発音可能性（平易性）・明確性・予測容易性・読み手網羅性が寄与する。理解容易性が *黙読* の速さなら、こちらは *対話・指示* のしやすさだ ── 冒頭の「AI への指示」という動機がここに効く。
<!-- 効率性 -->

### 安全性 (safety)

紛らわしい名前による *誤用・バグ* を防げること。`deleteAt` と `deletedAt`、List でないのに `List` ── 取り違えやすい名前は事故を生む。逆に `dangerouslySetInnerHTML` のように名前で危険を知らせれば誤用を未然に防げる（明確性が安全性に効く例）。
<!-- リスク回避性 -->
...
たとえばサイズ段階に名前を付けるとき、`largest` のような最上級・絶対語を選ぶと、より大きい段階が増えた瞬間に名前が嘘になり、改名を迫られる。`s`/`m`/`l`/`xl` のような相対スケールにしておけば、あとから `xxl` を足すだけで済み、既存の名前は無効化しない。適量性で *拡張余地* を残しておくほど、安定性は上がる ── いわば open-closed 原則の「拡張に開く」を命名で行うわけだ。

なお、公開した名前を約束として変えない *連続性* とは別物だ。連続性が「変えたくても変えない」discipline なのに対し、安定性は「そもそも変える必要が出ない」という結果を指す。
<!-- リスク回避性 -->

### 変更容易性 (modifiability)

いざ名前を変えるとき、*楽かつ安全に直せる* こと。安定性が「変えずに済む」を最大化するのに対し、こちらは「変えるときのコストを下げる」── ちょうど対になる観点だ。一意でよく弁別された名前は、grep で全使用箇所に漏れなく当たり（内部検索性）、似た名前と紛れず誤爆しない（弁別性）ので、リネーム（一括置換）が安全・確実になる。逆に `data` や `id` のような頻出語は、置換しようとすると無関係な箇所まで巻き込む。
<!-- リスク回避性＋効率性。modifiability は ISO だと保守性（製品側）だが、ここでは利用時の効果として借用 -->

### 読み手網羅性 (reader coverage)

*人・AI・新規参加者* の誰が読んでも通じること。冒頭で触れた「読み手は人間だけではない」がここに効く。
...
たとえば、「アンケート」の命名について。日本人は「アンケート」として馴染みがあるが、これはもともとフランス語。`enquete` のような変数名を書くと Code Spell Checker でスペルミス扱いされる（英語辞書で設定していれば）。英語では questionnaire と言います。
