---
title: "命名品質特性 ── 良い名前を測る3つの軸と15の性質"
emoji: "🏷️"
type: "tech"
...
名前のよさは、漠然とした「センス」ではなく、いくつもの観点に分けて整理できる。本記事ではそれを **命名品質特性** と呼び、ソフトウェア品質特性に倣って **3 つの軸** と **15 の性質** に整理する。15 の内訳は、3 つの軸で分類する **13**（名前そのものの品質）＋ 使った結果として現れる「利用時の品質」**2** だ（すべて語尾は「〜性」）。
...
名前が **文法・形式として正しい（適格である）** か。言語学では、文がルール上正しいことを「適格性（well-formedness）」と呼ぶ。これになぞらえている。英語と、使う言語・フレームワークを前提に判断する観点だ。関数・メソッドは動詞句、変数・クラスは名詞句。コレクションは複数形。真偽値は `is`/`has`/`can`。否定形は避けて肯定を基準に。`user.hasPermission()` が自然な英文として読めるか、といった具合だ。
